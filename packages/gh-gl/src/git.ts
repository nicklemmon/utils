...
/**
 * Initialize an empty git repository at `dir`, for use as a scratch working
 * directory during a sync run.
 *
...
/**
 * Fetch `ref` from `remoteUrl` into the scratch repo at `dir`. The fetched
 * commit becomes reachable as `FETCH_HEAD` in that repo.
 *
 * @param dir - A scratch repo previously created with {@link initScratchRepo}.
 * @param remoteUrl - A full git remote URL (or local path, for tests).
 * @param ref - The branch or ref to fetch.
 * @param options - `localRef` lands the fetched commit at that local ref
 *   (e.g. `refs/heads/main`) instead of the default `FETCH_HEAD`. Use this
 *   when fetching from two different remotes into the same scratch repo, so
 *   the second fetch doesn't overwrite the first's `FETCH_HEAD`. `shallow`
 *   fetches only `ref`'s tip commit (no history) — safe for the rebuild path,
 ...
 * Used before committing, so the new commit's parent is `ref`'s current
 * value while the worktree and index stay exactly what the caller placed
 * there (an extracted tree plus an overlay, not `ref`'s old content).
 *
 * @param dir - A scratch repo previously created with {@link initScratchRepo}.
...
/**
 * Check out `branch` into the scratch repo's worktree. Unlike
 * {@link setSymbolicHead}, this populates the worktree and index with
 * `branch`'s actual content — required before {@link mergeRef}, which needs
 * a real checkout to compute and apply a merge.
 *
...
/**
 * Attempt to merge `ref` into the branch currently checked out in the
 * scratch repo at `dir`. Never resolves conflicts automatically: on a
 * conflict, the merge is left in progress so the caller can inspect it, or
 * abort it with {@link abortMergeConflict}.
 *
...
/**
 * Abort an in-progress conflicted merge, leaving the branch exactly as it
 * was before {@link mergeRef} was called.
 *
...
/**
 * Extract the tree at `ref` into `destDir`, via `git archive | tar -x`. This
...
/**
 * Fingerprint the current content of `sourceDir` as a git tree hash, without
 * assuming `sourceDir` is itself a git checkout. Stages `sourceDir` into a
 * throwaway index backed by the scratch repo at `dir`, so this works for any
 * directory on disk (a git checkout, an extracted archive, anything).
 *
 * @param dir - A scratch repo previously created with {@link initScratchRepo}.
